...
import AllosoDetailViewer from '@/components/AllosoDetailViewer';
...
export default function TestAllosoPage() {
  const [products, setProducts] = useState<AllosoProduct[]>([]);
  const [selectedProduct, setSelectedProduct] = useState<AllosoProduct | null>(null);
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const response = await fetch('/api/test-alloso');
        if (!response.ok) {
    ...
    return (
      <div className="min-h-screen bg-white">
        <div className="max-w-6xl mx-auto px-4 py-8">
          ...
          <AllosoDetailViewer product={selectedProduct} />
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-12">
      <div className="max-w-4xl mx-auto px-4">
        <h1 className="text-3xl font-bold text-gray-900 mb-8">Alloso Products Test</h1>
        ...
        <div className="bg-white rounded-lg shadow-md p-6 mb-8">
          ...
          <div className="space-y-3">
            ...
            <div>
              <span className="font-medium text-gray-700">Detail Content:</span>
              <ul className="text-gray-600 mt-1 space-y-1">
                <li>Image 1: {selectedProduct.detailImage1 ? '✓' : '✗'}</li>
                <li>Text 1: {selectedProduct.detailText1 ? '✓' : '✗'}</li>
                <li>Image 2: {selectedProduct.detailImage2 ? '✓' : '✗'}</li>
                <li>Text 2: {selectedProduct.detailText2 ? '✓' : '✗'}</li>
              </ul>
            </div>
          </div>
        </div>

...
export default function HanssemProductsPage() {
  ...
  useEffect(() => {
    ...
    async function loadProducts() {
      try {
        ...
        // scraped-products 폴더의 모든 JSON 파일 가져오기
        const response = await fetch('/api/hanssem-products');
        const products = await response.json();

        setAllProducts(products);
      } catch (error) {
        console.error("한샘 제품 로드 실패:", error);
      } finally {
        setLoading(false);
      }
    }
  ...
  const handleProductClick = (product: any) => {
    router.push(`/hanssem-detail/${product.productCode}`);
  };
      ...
      <main className="px-4 lg:px-8 pb-20">
        <div className="max-w-[1400px] mx-auto">
          {loading ? (
            ...
            <div className="flex items-center justify-center min-h-[400px]">
              <div className="text-center text-gray-400">
                <p className="text-lg">해당 카테고리에 제품이 없습니다</p>
              </div>
            </div>
          ) : (
            <div className="grid grid-cols-2 lg:grid-cols-4 xl:grid-cols-5 gap-4 lg:gap-6 mt-8">
              {filteredProducts.map((product: any, index: number) => (
                <div
                  ...
                  className="bg-white rounded-lg overflow-hidden shadow-sm hover:shadow-xl transition-all cursor-pointer border border-gray-100 hover:border-red-300"
                >
                  {/* 제품 이미지 */}
                  ...
                  <div className="p-3 lg:p-4">
                    <div className="inline-block px-2 py-1 bg-red-50 rounded text-xs text-red-600 font-medium mb-2">
                      한샘
                    </div>

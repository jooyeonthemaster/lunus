...
// 일룸 제품 데이터 import
import iloomGuesil from "../../../data/일룸/iloom-거실.json";
import iloomSeojae from "../../../data/일룸/iloom-서재.json";
import iloomOtjang from "../../../data/일룸/iloom-옷장.json";
import iloomJomyeong from "../../../data/일룸/iloom-조명.json";
import iloomJubang from "../../../data/일룸/iloom-주방.json";
import iloomKidsroom from "../../../data/일룸/iloom-키즈룸.json";
import iloomHakseang from "../../../data/일룸/iloom-학생방.json";

export default function IloomProductsPage() {
  ...
  useEffect(() => {
    // 모든 일룸 제품 합치기
    const combined = [
      ...iloomGuesil.map((p: any) => ({ ...p, category: "거실" })),
      ...iloomSeojae.map((p: any) => ({ ...p, category: "서재" })),
      ...iloomOtjang.map((p: any) => ({ ...p, category: "옷장" })),
      ...iloomJomyeong.map((p: any) => ({ ...p, category: "조명" })),
      ...iloomJubang.map((p: any) => ({ ...p, category: "주방" })),
      ...iloomKidsroom.map((p: any) => ({ ...p, category: "키즈룸" })),
      ...iloomHakseang.map((p: any) => ({ ...p, category: "학생방" })),
    ];
    setAllProducts(combined);
  }, []);

  // 카테고리 필터링 + 가격이 정상인 제품만 (1000원 이상)
  const filteredProducts =
    selectedCategory === "전체"
      ? allProducts.filter((p) => p.price > 1000)
      : allProducts.filter((p) => p.category === selectedCategory && p.price > 1000);

  const handleProductClick = (product: any) => {
    // 제품 URL에서 productCd 추출
    const urlParams = new URLSearchParams(product.productUrl.split("?")[1]);
    const productCd = urlParams.get("productCd");
    
    if (productCd) {
      router.push(`/iloom-detail/${productCd}`);
    }
  };

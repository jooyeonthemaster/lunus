...
import InartDetailViewer from '@/components/InartDetailViewer';

interface InartProduct {
  title: string;
  price?: number;
  productUrl: string;
  imageUrl: string;
...
export default function TestInartPage() {
  const [products, setProducts] = useState<InartProduct[]>([]);
  const [selectedProduct, setSelectedProduct] = useState<InartProduct | null>(null);
  const [showSelector, setShowSelector] = useState(true);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    fetch('/api/test-inart')
      .then(res => res.json())
      .then(data => {
        if (data.success) {
      ...
      <div className="min-h-screen bg-gray-50 p-8">
        <div className="max-w-4xl mx-auto">
          <div className="bg-white rounded-xl shadow-sm p-8 mb-6">
            <div className="mb-6">
              <h1 className="text-3xl font-light text-gray-900 mb-2">인아트 제품 뷰어</h1>
            ...
            {selectedProduct && (
              <div className="mt-6 p-4 bg-gray-50 rounded-lg">
                ...
                <div className="space-y-2 text-sm text-gray-600">
                  ...
                  <p><span className="font-medium">상세 이미지:</span> {selectedProduct.detailImage ? '있음' : '없음'}</p>
                </div>
              </div>
            )}
